...
2. **Demo PDF (DynamoDB Local)** — o `docker-compose.yml` sobe `dynamodb-local` e o `analytics-service` usa `DYNAMODB_ENDPOINT_URL`. Para o demo local, `AWS_SQS_URL` pode ficar vazio (o worker de SQS so inicia se `AWS_SQS_URL` estiver definido).
3. **Kubernetes** — Ver `k8s/README.md` (ECR via `scripts/render-k8s-manifests.sh`, ElastiCache para evaluation, `SERVICE_API_KEY` no `secret-evaluation.yml`).
